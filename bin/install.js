...
console.log(
	cyan("[info]") + `  Installing opencode-live-compaction into ${targetDir}`,
);
...
if (existsSync(configFile)) {
	...
	if (content.includes("opencode-live-compaction")) {
		console.log(
			green("[ok]") + "    opencode.json already references the plugin",
		);
	} else {
		console.log(
			green("[ok]") +
				"    Local plugin is already active — no config change needed.",
		);
	}
} else {
	console.log(
		cyan("[info]") +
			"  The local plugin will be auto-loaded from .opencode/plugins/live-compaction.ts",
	);
}
...
console.log(
	green("[ok]") +
		"    Done! OpenCode will use enhanced 11-section compaction on next session.",
);
